...
void MainWindow::on_pushButton_clicked()
{
    ...
    ui->statusLabel->setText("Filenames read");
    std::cout<<"Filenames read" << std::endl;
    ...
    std::cerr << fixedImageReader->GetOutput()->GetLargestPossibleRegion().GetSize() << std::endl;
    PrintSlice(fixedImageReader->GetOutput());
    std::cout<<"Slice printed" << std::endl;
    //ui->statusLabel->setText(ui->statusLabel->text() + "\nSlice printed");
    ...
    ui->statusLabel->setText(ui->statusLabel->text() + "Initial transform set");

    // Здесь можно настроить параметра работы с объёмами
    ...
    ui->statusLabel->setText(ui->statusLabel->text() + "Optimizer set");
    ...
    ui->statusLabel->setText(ui->statusLabel->text() + "Smoothing set");
    try
    {
...
{
    QString def = "C:/SIFT/tomography/data/shifted";
    QDir path = QDir(def);
    QString srcdir = QFileDialog::getExistingDirectory(this, "Выберите директорию исходника", path.path());
